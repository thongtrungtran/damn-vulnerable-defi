...
describe("[Challenge] Backdoor", function () {
  ...
  it("Exploit", async function () {
    ...
    const WalletRegistryAttacker = await ethers.getContractFactory(
      "WalletRegistryAttacker",
      attacker
    );
    this.attackerContract = await WalletRegistryAttacker.deploy(
      this.walletRegistry.address,
      users,
      this.token.address,
      this.walletFactory.address,
    ...
    await this.attackerContract.exploit({ gasLimit: 2e6 });
  });
